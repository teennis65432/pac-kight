...
public class Wisp extends Ghost {	//For explanation, refer to Necromancer.java
	...
	Animation wispUp, wispDown, wispLeft, wispRight;
	Animation eWispUp, eWispDown, eWispLeft, eWispRight;
	...
	public void init(){ //Initializing the class and variables. 
		...
		//Animations
		Image[] wiri = {chars.getSprite(5, 12), chars.getSprite(6, 12)};
		wispRight = new Animation(wiri, delta);
		wispRight.setPingPong(true);
		
		Image[] wili = {chars.getSprite(5, 13), chars.getSprite(6, 13)};
		wispLeft = new Animation(wili, delta);
		wispLeft.setPingPong(true);
		
		Image[] widi = {chars.getSprite(5, 14), chars.getSprite(6, 14)};
		wispDown = new Animation(widi, delta);
		wispDown.setPingPong(true);
		
		Image[] wiui = {chars.getSprite(5, 15), chars.getSprite(6, 15)};
		wispUp = new Animation(wiui, delta);
		wispUp.setPingPong(true);
		
		Image[] ewiri = {chars.getSprite(5, 16), chars.getSprite(6, 16)};
		eWispRight = new Animation(ewiri, delta);
		eWispRight.setPingPong(true);
		
		Image[] ewili = {chars.getSprite(5, 17), chars.getSprite(6, 17)};
		eWispLeft = new Animation(ewili, delta);
		eWispLeft.setPingPong(true);
		
		Image[] ewidi = {chars.getSprite(5, 18), chars.getSprite(6, 18)};
		eWispDown = new Animation(ewidi, delta);
		eWispDown.setPingPong(true);
		
		Image[] ewiui = {chars.getSprite(5, 19), chars.getSprite(6, 19)};
		eWispUp = new Animation(ewiui, delta);
		eWispUp.setPingPong(true);
		...
		if(state != State.DEATH){	//Allows it to do stuff if not dead
			if(!Scatterbrain){		//If the Extras option "Scatterbrain" was turned off
				if(dirTimeout.getTime() > 1f){
						targetAxisX = Adventure.knight.centerX;
						targetAxisY = Adventure.knight.centerY;
				}
				
				targetLine.set(centerX, centerY, targetAxisX, targetAxisY);
				if(Math.abs(targetLine.length()) > 64){
